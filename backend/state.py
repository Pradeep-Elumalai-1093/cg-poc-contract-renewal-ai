...
def generate_contracts() -> list[dict]:
    ...
    seq = 1
    name_idx = 0
    region_counts = {"NATT": 14, "ETT": 14, "APAC_TT": 10}
    ...
    for region_id, count in region_counts.items():
        ...
        for _ in range(count):
            ...
            contracts.append({
                ...
                "customerId": f"CU-{seq:04d}",
                "customerName": FLEET_NAMES[name_idx % len(FLEET_NAMES)],
                "region": region_id,
                "channel": channel,
                "dealerId": f"DLR-{100 + (seq % 12)}" if channel == "Dealer" else None,
